    ...
    $.ajax({
        ...
        success:function(data, textStatus, jqXHR)
        {
            if (data.success) {
                ...
                var count = parseInt(form.parent().parent().parent().children('.post-header').children('.count').html()) + 1;
                form.parent().parent().parent().children('.post-header').children('.count').html(count);
